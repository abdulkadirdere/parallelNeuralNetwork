...
void main(int argc, char *argv[]) {
    int num_input_nodes = 2;                                      // number of input layer nodes - columns
    int num_hidden_nodes = 3;                                     // number of hidden layer nodes - rows
    ...
    double *local_output_nodes = malloc(num_output_nodes * sizeof(double));
    ...
    createMatrix("Layer 1 Weights", local_hidden_weights, row, rows_per_process, col, rank, comm);
    printMatrix("Layer 1 Weights",local_hidden_weights, row, rows_per_process, col, rank, comm);
    
    // create input nodes data
    createVector("Input Node", local_input_nodes, col, columns_per_process, rank, comm);
    printArray("Input Node",local_input_nodes, col, columns_per_process, rank, comm);
    ...
    printArray("Matrix Multiplication",local_hidden_nodes, row, rows_per_process, rank, comm);
