...
int main() {
    // initialisation
    int num_input_nodes = 300; // number of input layer nodes
    int num_hidden_nodes = 400; // number of hidden layer nodes
    ...
    //-------------- Serial Feed-Forward Neural Network --------------//
    ...
    //-------------- CUDA Feed-Forward Neural Network --------------//
    ...
    float num_ops = num_input_nodes * num_hidden_nodes * BLOCK_SIZE * BLOCK_SIZE; // size of the image (width * height) * size of mask (3*3)

    float serial_throughput = num_ops / (serial_time / 1000.0f) / 1000000000.0f;
    float cuda_throughput = num_ops / (cuda_time / 1000.0f) / 1000000000.0f;

    std::cout << "Input Nodes: " << num_input_nodes << " Hidden Nodes:" << num_hidden_nodes << " Output Nodes:" << num_output_nodes << std::endl;

    printf("\nSerial Feed-Forward Neural Network Time: %3.2f ms \n", serial_time);
    printf("CUDA Feed-Forward Neural Network Time: %3.2f ms \n", cuda_time);

    std::cout << "\nSpeedup of CUDA kernel (CPU/GPU): " << serial_time / cuda_time << " ms" << std::endl;
      
    std::cout << "\nThroughput of serial implementation: " << serial_throughput << " GFLOPS" << std::endl;
    std::cout << "Throughput of CUDA kernel implementation: " << cuda_throughput << " GFLOPS" << std::endl;

    std::cout << "\nPerformance improvement: CUDA over serial " <<  cuda_throughput / serial_throughput << "x" << std::endl;
